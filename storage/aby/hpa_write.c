...
static HASH_INFO *hpa_find_free_hash(HPA_SHARE *info, HPA_BLOCK *block,
				     ulong records);
...
#endif
  if (share->auto_key)
    aby_update_auto_increment(info, record);
  DBUG_RETURN(0);
...
int hpa_rb_write_key(HPA_INFO *info, HPA_KEYDEF *keyinfo, const uchar *record, 
		    uchar *recpos)
{
 ...
 **/
  ...
  if (!(block_pos=(info->records % info->block.records_in_block)))
  {
    ...
    if (hpa_get_new_block(&info->block,&length))
      DBUG_RETURN(NULL);
...
/*
  ...
  SYNOPSIS
    ...
    recpos   Memory buffer where the table record will be stored if added 
             successfully
  NOTE
    Hash index uses HPA_BLOCK structure as a 'growable array' of HASH_INFO 
    structs. Array size == number of entries in hash index.
    hpa_mask(hpa_rec_hashnr()) maps hash entries values to hash array positions.
  ...
  RETURN
    ...
    still added and the caller must call hpa_delete_key for it.
*/
		 ...
		 const uchar *record, uchar *recpos)
{
  ...
  pos= hpa_find_hash(&keyinfo->block,(first_index=share->records-halfbuff));
  ...
  /*
    ...
    guaranteed by properties of hpa_mask(hpa_rec_hashnr(X)) mapping function)
  ...
  */
  if (pos != empty)				/* If some records */
  {
    do
    {
      hashnr = hpa_rec_hashnr(keyinfo, pos->ptr_to_rec);
      if (flag == 0)
      {
        /* 
          ...
          comment
        */
	if (hpa_mask(hashnr, share->blength, share->records) != first_index)
	  break;
      }
      /*
  ...
  pos=hpa_find_hash(&keyinfo->block, hpa_mask(hpa_rec_hashnr(keyinfo, record),
  ...
  else
  {
    ...
    empty[0]=pos[0];
    gpos=hpa_find_hash(&keyinfo->block,
    ...
    else
    {
      ...
      pos->ptr_to_rec=recpos;
      pos->next_key=0;
      hpa_movelink(pos, gpos, empty);
    }

    /* Check if duplicated keys */
    if ((keyinfo->flag & HA_NOSAME) && pos == gpos &&
	(!(keyinfo->flag & HA_NULL_PART_KEY) ||
	 !hpa_if_null_in_key(keyinfo, record)))
    {
      pos=empty;
      do
      {
	if (! hpa_rec_key_cmp(keyinfo, record, pos->ptr_to_rec, 1))
	{
...
static HASH_INFO *hpa_find_free_hash(HPA_SHARE *info,
				     HPA_BLOCK *block, ulong records)
{
  uint block_pos;
  size_t length;

  if (records < block->last_allocated)
    return hpa_find_hash(block,records);
  if (!(block_pos=(records % block->records_in_block)))
  {
    if (hpa_get_new_block(block,&length))
      return(NULL);
