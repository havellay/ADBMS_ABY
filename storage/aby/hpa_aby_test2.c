...
#include "abydef.h"		/* Because of hpa_find_block */
#include <signal.h>
...
int main(int argc, char *argv[])
{
  ...
  HPA_INFO *file,*file2;
  HPA_SHARE *tmp_share;
  HPA_KEYDEF keyinfo[MAX_KEYS];
  HA_KEYSEG keyseg[MAX_KEYS*5];
  ABY_PTR UNINIT_VAR(position);
  ...
  hpa_create_info.keys= keys;
  hpa_create_info.keydef= keyinfo;
  hpa_create_info.reclength= reclength;
  hpa_create_info.max_records= (ulong) flag*100000L;
  hpa_create_info.min_records= (ulong) recant/2;
  ...
  for (i=0 ; i < recant ; i++)
  {
    ...
    if (testflag == 1 && aby_check_aby(file,0))
    {
  ...
  for (i=0 ; i < write_count/10 ; i++)
  {
    ...
    if (j != 0)
    {
      ...
      if (aby_rkey(file,record,0,key,6, HA_READ_KEY_EXACT))
      {
	printf("can't find key1: \"%s\"\n",(char*) key);
	goto err;
      }
      if (aby_delete(file,record))
      {
	...
	goto err;
      ...
      if (testflag == 2 && aby_check_aby(file,0))
      {
	puts("Heap keys crashed");
	goto err;
      }
  ...
  for (i=0 ; i < write_count/10 ; i++)
  {
    ...
    if (rnd(2) == 1)
    {
      if (aby_scan_init(file))
	goto err;
      j=rnd(write_count-opt_delete);
      while ((error=aby_scan(file,record) == HA_ERR_RECORD_DELETED) ||
	     (!error && j))
      {
	if (!error)
	  j--;
    ...
    if (aby_update(file,record,record2))
    {
      if (my_errno != HA_ERR_FOUND_DUPP_KEY || key3[n3] == 0)
      {
    ...
    if (testflag == 3 && aby_check_aby(file,0))
    {
      puts("Heap keys crashed");
      goto err;
    }
  ...
  if (dupp_keys > 3)
  {
    ...
    if (aby_rkey(file,record,0,key,6, HA_READ_KEY_EXACT))
      goto err;
    if (aby_rnext(file,record3)) goto err;
    if (aby_delete(file,record3)) goto err;
    key_check-=atoi((char*) record3);
    key1[atoi((char*) record+keyinfo[0].seg[0].start)]--;
    key3[atoi((char*) record+keyinfo[2].seg[0].start)]=0;
    opt_delete++;
    ant=2;
    while ((error=aby_rnext(file,record3)) == 0 ||
    ...
    if (aby_rlast(file,record3,0)) goto err;
    if (aby_delete(file,record3)) goto err;
    key_check-=atoi((char*) record3);
    key1[atoi((char*) record+keyinfo[0].seg[0].start)]--;
    key3[atoi((char*) record+keyinfo[2].seg[0].start)]=0;
    opt_delete++;
    if (aby_rprev(file,record3) || aby_rprev(file,record3))
      goto err;
    if (aby_delete(file,record3)) goto err;
    key_check-=atoi((char*) record3);
    key1[atoi((char*) record+keyinfo[0].seg[0].start)]--;
    ...
    opt_delete++;
    ant=3;
    while ((error=aby_rprev(file,record3)) == 0 ||
	   error == HA_ERR_RECORD_DELETED)
    {
      if (! error)
	ant++;
  ...
  if (aby_scan_init(file))
    goto err;
  while ((error=aby_scan(file,record3) == HA_ERR_RECORD_DELETED)) ;
  if (error)
    goto err;
  if (aby_delete(file,record3)) goto err;
  key_check-=atoi((char*) record3);
  ...
  key3[atoi((char*) record+keyinfo[2].seg[0].start)]=0;
  ant=0;
  while ((error=aby_scan(file,record3)) == 0 ||
	 error == HA_ERR_RECORD_DELETED)
    if (! error)
      ant++;
	 ...
	 (error == 0 && pos))
  {
    ...
    if (!error && (i-- == 0))
    {
      ...
      position=aby_position(file);
    }
  }
  if (error)
    goto err;
  bmove(record2,record,reclength);
  if (aby_rsame(file,record,-1) || aby_rsame(file,record2,2))
    goto err;
  if (memcmp(record2,record,reclength))
  {
    puts("aby_rsame didn't find right record");
    goto end;
  }

  puts("- Test of read through position");
  if (aby_rrnd(file,record,position))
    goto err;
  if (memcmp(record3,record,reclength))
  {
    puts("aby_frnd didn't find right record");
    goto end;
  }
...
#ifdef OLD_ABY_VERSION
  {
    ...
    if (aby_extra(file,HA_EXTRA_RESET) || aby_extra(file,HA_EXTRA_CACHE))
    {
      puts("got error from aby_extra");
      goto end;
    }
    ant=check=0;
    while ((error=aby_rrnd(file,record,(ulong) -1)) != HA_ERR_END_OF_FILE &&
	   ant < write_count + 10)
    {
      if (!error)
      {
    ...
    if (aby_extra(file,HA_EXTRA_NO_CACHE))
    {
      puts("got error from aby_extra(HA_EXTRA_NO_CACHE)");
      goto end;
    }
  }
#endif
  ...
  if (aby_reset(file) || aby_extra(file,HA_EXTRA_CACHE))
  {
    puts("got error from aby_extra");
    goto end;
  }
  ant=check2=0;
  aby_scan_init(file);
  while ((error=aby_scan(file,record)) != HA_ERR_END_OF_FILE &&
	 ant < write_count + 10)
  {
    if (!error)
    {
  ...
  ant=0;
  for (error=aby_rkey(file,record,0,key,6, HA_READ_KEY_EXACT);
      ! error ;
       error=aby_rnext(file,record))
    ant++;
  if (ant != dupp_keys)
  {
  ...
  ant=0;
  for (error=aby_rlast(file,record,0) ;
      ! error ;
      error=aby_rprev(file,record))
  {
    ant++;
    check2+=calc_check(record,reclength);
  }
  ...
  while ((error=aby_scan(file,record)) != HA_ERR_END_OF_FILE)
  {
    if (error == 0)
    {
      if (aby_rkey(file2,record2,2,record+keyinfo[2].seg[0].start,8,
		    HA_READ_KEY_EXACT))
      {
  ...
  printf("- Creating output aby-file 2\n");
  hpa_create_info.keys= 1;
  hpa_create_info.max_records= 0;
  hpa_create_info.min_records= 0;
  if (aby_create(filename2, &hpa_create_info, &tmp_share, &unused) ||
      !(file2= aby_open_from_share_and_register(tmp_share, 2)))
    goto err;
  ...
  printf("- Checking aby tables\n");
  if (aby_check_aby(file,1) || aby_check_aby(file2,1))
  {
    puts("Heap keys crashed");
    goto err;
  }
  ...
  aby_clear(file);
  aby_clear(file2);
  if (aby_close(file) || (file2 && aby_close(file2)))
    goto err;
  aby_delete_table(filename2);
  hpa_panic(HA_PANIC_CLOSE);
  my_end(MY_GIVE_INFO);
  return(0);
err:
