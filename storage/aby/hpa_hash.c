                               ...
                               key_range *max_key)
{
  ...
  HPA_KEYDEF *keyinfo= info->s->keydef + inx;
  ...
  if (min_key)
  {
    custom_arg.key_length= hpa_rb_pack_key(keyinfo, (uchar*) info->recbuf,
					  (uchar*) min_key->key,
					  min_key->keypart_map);
...
uchar *hpa_search(HPA_INFO *info, HPA_KEYDEF *keyinfo, const uchar *key,
                uint nextflag)
{
  ...
  if (share->records)
  {
    pos=hpa_find_hash(&keyinfo->block, hpa_mask(hpa_hashnr(keyinfo, key),
					      share->blength, share->records));
    do
    {
      if (!hpa_key_cmp(keyinfo, pos->ptr_to_rec, key))
      {
	switch (nextflag) {
	...
	case 3:					/* Search same */
      ...
      {
	flag=0;					/* Reset flag */
	if (hpa_find_hash(&keyinfo->block,
			 hpa_mask(hpa_rec_hashnr(keyinfo, pos->ptr_to_rec),
...
uchar *hpa_search_next(HPA_INFO *info, HPA_KEYDEF *keyinfo, const uchar *key,
		      HASH_INFO *pos)
{
...
ulong hpa_mask(ulong hashnr, ulong buffmax, ulong maxlength)
{
...
void hpa_movelink(HASH_INFO *pos, HASH_INFO *next_link, HASH_INFO *newlink)
{
  HASH_INFO *old_link;
  do
  {
...
ulong hpa_hashnr(register HPA_KEYDEF *keydef, register const uchar *key)
{
  /*register*/ 
  ulong nr=1, nr2=4;
...
ulong hpa_rec_hashnr(register HPA_KEYDEF *keydef, register const uchar *rec)
{
  ulong nr=1, nr2=4;
  HA_KEYSEG *seg,*endseg;
...
int hpa_rec_key_cmp(HPA_KEYDEF *keydef, const uchar *rec1, const uchar *rec2,
...
int hpa_key_cmp(HPA_KEYDEF *keydef, const uchar *rec, const uchar *key)
{
  HA_KEYSEG *seg,*endseg;
...
void hpa_make_key(HPA_KEYDEF *keydef, uchar *key, const uchar *rec)
{
  HA_KEYSEG *seg,*endseg;
...
uint hpa_rb_make_key(HPA_KEYDEF *keydef, uchar *key, 
...
uint hpa_rb_pack_key(HPA_KEYDEF *keydef, uchar *key, const uchar *old,
                    key_part_map keypart_map)
{
       ...
       seg < endseg && keypart_map; old+= seg->length, seg++)
  {
    ...
    if (seg->null_bit)
    {
      /* Convert NULL from MySQL representation into ABY's. */
      if (!(*key++= (char) 1 - *old++))
      {
        /*
          Skip length part of a variable length field.
          Length of key-part used with aby_rkey() always 2.
          See also hpa_hashnr().
        */
        if (seg->flag & (HA_VAR_LENGTH_PART | HA_BLOB_PART))
          old+= 2;
...
uint hpa_rb_key_length(HPA_KEYDEF *keydef, 
...
uint hpa_rb_null_key_length(HPA_KEYDEF *keydef, const uchar *key)
{
...
uint hpa_rb_var_key_length(HPA_KEYDEF *keydef, const uchar *key)
{
  const uchar *start_key= key;
  HA_KEYSEG *seg, *endseg;
...
my_bool hpa_if_null_in_key(HPA_KEYDEF *keydef, const uchar *record)
{
  HA_KEYSEG *seg,*endseg;
  for (seg=keydef->seg,endseg=seg+keydef->keysegs ; seg < endseg ; seg++)
  {
...
void aby_update_auto_increment(HPA_INFO *info, const uchar *record)
{
