              ...
              key_part_map keypart_map, enum ha_rkey_function find_flag)
{
  ...
  info->lastinx= inx;
  info->current_record= (ulong) ~0L;		/* For aby_rrnd() */
  ...
  if (keyinfo->algorithm == HA_KEY_ALG_BTREE)
  {
    ...
    custom_arg.key_length= info->lastkey_len= 
      hpa_rb_pack_key(keyinfo, (uchar*) info->lastkey,
  ...
  {
    if (!(pos= hpa_search(info, share->keydef + inx, key, 0)))
    {
      info->update= 0;
...
uchar* aby_find(HPA_INFO *info, int inx, const uchar *key)
{
  return hpa_search(info, info->s->keydef + inx, key, 0);
}
